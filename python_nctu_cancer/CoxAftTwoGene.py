#負責Cox regression和AFT 的 two genes分析結果
import sys
import traceback
import os
import base64
...
class CoxAftTwoGene:
    ...
    #交集臨床資料的patienID後，把第一個基因的表現量parse過去
    def get_exp1(self, df1, df2):
        ...
        return df1
    #交集臨床資料的patienID後，把第二個基因的表現量parse過去
    def get_exp2(self, df1, df2):
        ...
        return df1
    #做Cox和AFT時，必須所有的variable都有資料，清除空值。
    def drop_nan(self, df1):
        ...
        return df1
    #執行Cox，回傳coefficient和p-value（表格上的數值）
    def cox(self, df, sur):
    ...
    #Cox regression的Hazard ratio plot
    ...
    #執行AFT，回傳coefficeint和p-value（表格上的數值）
    def aft(self, df, sur):
    ...
    def aft_img(self, df, sur, gene1, gene2, drop_image_columns, cancer_type):
        ...
        return [html, tmps]
    #對第一個目標基因的表現量做Inverse Normal Transformation
    def transform1(self, df, num):
        ...
        return df
    #對第二個目標基因的表現量做Inverse Normal Transformation
    ...
    # 取得cox檔案路徑，221主機上 （/home/haowei/python_nctu_cancer/python_nctu_cancer/Cox_parsedata）
    def get_cox_path(self, target_path, target_file):
        ...
        return None
    #針對不同存活指標（OS, DFI, PFI, DSS）做Cox分析，回傳網頁上表格內容
    def get_cox_data(self, category1, category2, gene1, gene2, cancer_type):
        ...
        for mode in modes:
            ...
            try:
                ...
                new_df = self.get_exp1(clinical_info, g1_exp_df[g1_exp_df[index] == gene1])
                new_df = self.get_exp2(new_df, g2_exp_df[g2_exp_df[index] == gene2])
        ...
        return res
    #負責Cox regression的Hazard ratio plot和旁邊小表格的驅動
    def get_cox_img(self, mode, category1, category2, gene1, gene2, cancer_type, drop_image_columns):
        ...
        try:
            ...
            new_df = self.get_exp1(clinical_info, g1_exp_df[g1_exp_df['MetaFeature'] == gene1])
            new_df = self.get_exp2(new_df, g2_exp_df[g2_exp_df[index] == gene2])

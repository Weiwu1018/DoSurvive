#負責More than 2 genes的Cox和AFT的繪製 
import sys
import traceback
import pandas as pd
import statistics
...
class MoreGene:
    ...
    #針對取出目標基因的表現量做patientID交集後，將表現量join到臨床資料
    def get_exp1(self, df1, df2, gene):
        ...
        return df1
    #lncRNA因為名稱轉換的緣故，另寫一個將表現量Join的function
    ...
    def drop_nan(self, df1, gene):
        ...
        return df1
    #執行Cox，回傳p-value, coefficient（主要包含大表格的數值）
    ...
    def aft_img(self, df, sur, list, drop_image_columns):
        ...
        return [html, tmp_df]
    #表現量做Inverse Normal Transformation
    ...
    # 取得cox檔案路徑 (/home/haowei/python_nctu_cancer/python_nctu_cancer/Cox_parsedata)
    ...
    def get_aft_img_data(self, cancer_type, mode, gene_mrnas, gene_mirnas, gene_lncrnas, drop_image_columns):
        ...
        return data
    #Cox regression上傳表格重新分析的function
    ...
    def get_aft_upload_img_data(self, cancer_type, mode, gene_mrnas, gene_mirnas, gene_lncrnas, upload_file):
        ...
        return data
    #不管是Cox或是AFT下載下來的表格都是一樣的，都是這個function負責

#負責單基因的Cox regression和AFT model結果輸出
...
class NewAft:
    ...
    #下載下來的資料要保留PatientID用的（並且修改下載下來的column name）
    ...
    def count(self, cancer_type, dic, survival_type):
        ...
        if df1.empty:
            raise Exception("DataFrame is empty")
        #表現量做Inverse Normal Transfomation
        ex = df1['Expression']
        ex = pd.to_numeric(ex)
        newex = ex.rank(method='first').tolist()
